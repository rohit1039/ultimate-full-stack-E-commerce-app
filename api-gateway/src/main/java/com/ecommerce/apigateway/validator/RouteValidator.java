...
/**
 * A class that contains a list of open API endpoints and a predicate to determine if a request is
 * secured.
 */
@Component
public class RouteValidator {
  ...
  /**
   * A predicate that determines if a request is secured based on the endpoint and HTTP method.
   * return true if the request is not protected by authentication, false otherwise
   */
  public Predicate<ServerHttpRequest> isSecured =
      request ->
          endPoints.stream()
              .noneMatch(
                  uri ->
                      (request.getURI().getPath().contains(endPoints.get(0))
