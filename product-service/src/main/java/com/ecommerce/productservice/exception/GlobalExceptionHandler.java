...
public class GlobalExceptionHandler extends ResponseEntityExceptionHandler {
	...
	public ResponseEntity<ExceptionInResponse> handleProductNotFoundException(ProductNotFoundException exception) {
		...
		ExceptionInResponse exceptionInResponseForProduct = new ExceptionInResponse(404, exception.getMessage(),
				errorDescription);
		LOGGER.error("*** {} ***", exceptionInResponseForProduct.getErrorMessage());
		return new ResponseEntity<>(exceptionInResponseForProduct,
	...
	public ResponseEntity<ExceptionInResponse> handleDuplicateProductException(
		...
		ExceptionInResponse exceptionInResponse = new ExceptionInResponse(409, duplicateProductException.getMessage(),
				"Please try " + "with different product name");
		LOGGER.error("*** {} ***", exceptionInResponse.getErrorDescription());
		return new ResponseEntity<>(exceptionInResponse, HttpStatusCode.valueOf(exceptionInResponse.getErrorCode()));
	}
	...
	/**
	 * This function is used to handle the MethodArgumentNotValidException exception. It
	 * is a Spring-specific exception that is thrown when the request parameters do not
	 * match the method arguments. This function iterates through the field and global
	 * errors of the MethodArgumentNotValidException and adds them to a map, where the key
	 * is the field name and the value is the error message. It then returns a response
	 * with the map as the body and a status code of BAD_REQUEST.
	 * @param ex The MethodArgumentNotValidException exception that was thrown
	 * @param headers The headers of the request
	 * @param status The status code of the response
	 * @param request The web request
	 * @return A response with the error messages in a map as the body and a status code
	 * of BAD_REQUEST
	 */
	@Override
