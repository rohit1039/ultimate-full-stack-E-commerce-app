...
@Tag(name = "Product Service",
		description = "Clients should use this service to get product's details only, "
				+ "and Admins should use this service to add, get, update and delete product details")
@Validated
public class ProductServiceController {
	...
	public ResponseEntity<EntityModel<ProductResponseDTO>> addProduct(
			...
			@Parameter(in = ParameterIn.PATH,
					description = "id of the category to save the products") @PathVariable Integer categoryId)
			throws Exception {

		ProductResponseDTO productResponseDTO = this.productService.saveProductToDB(productRequestDTO, categoryId,
				username, role);
	 ...
	 * @return the product with the specified ID, or an error response if the product
	 * could not be found
	 */
	@Operation(summary = "Get product by Id",
			description = "A GET request to get product by Id, accessible by <b> CLIENTS & ADMINS </b>",
			tags = { "Product " + "Service" })
	@ApiResponses(value = { @ApiResponse(responseCode = "200", description = "Successfully found the product"),
			@ApiResponse(responseCode = "400", description = "Input validation failed"),
	...
	public ResponseEntity<EntityModel<ProductResponseDTO>> getProductById(
			@Parameter(in = ParameterIn.PATH, description = "id of the product") @PathVariable Integer productId) {
	...
	@Operation(summary = "Get all products",
			description = "A GET request to get all products, accessible by <b> CLIENTS & ADMINS </b>",
			tags = { "Product " + "Service" })
	@ApiResponses(value = { @ApiResponse(responseCode = "200", description = "Successfully found the products"),
			@ApiResponse(responseCode = "400", description = "Input validation failed"),
			@ApiResponse(responseCode = "401", description = "Unauthorized user"),
	...
	public ResponseEntity<CollectionModel<ProductResponseDTO>> getAllProducts(
			@Parameter(in = ParameterIn.QUERY,
					description = "the page number to retrieve products if there are more than one page of data",
					schema = @Schema(minimum = "1", defaultValue = "1")) @RequestParam(required = false,
							defaultValue = "1", value = "page") @Min(value = 1) int pageNumber,
			@Parameter(in = ParameterIn.QUERY,
					description = "the number of items per page, or the maximum number of products listed in response",
					schema = @Schema(minimum = "5", maximum = "20", defaultValue = "5")) @RequestParam(required = false,
							defaultValue = "5", value = "size") @Min(value = 5) @Max(value = 20) int pageSize,
			@Parameter(in = ParameterIn.QUERY, description = "search keyword to search for products") @RequestParam(
					required = false, defaultValue = "") String searchKey,
			@Schema(hidden = true) @RequestHeader(name = "userRole") String role) throws JsonProcessingException {

		Page<ProductResponseDTO> page = this.productService.getAllProducts(pageNumber, pageSize, searchKey, role);
		...
		CollectionModel<ProductResponseDTO> response = addPageMetadata(page.getContent(), page, 0, searchKey, role);
		return new ResponseEntity<>(response, HttpStatus.OK);
	}
	 ...
	 * <p>
	 * A GET request to get a list of products tagged to a specific category
	 * @param categoryId the id of the category to filter products by
	 * @return a list of products in the specified category
	 */
	@Operation(summary = "Get products by category",
			description = "A GET request to get a list of products tagged to a specific category, "
	...
	public ResponseEntity<CollectionModel<ProductResponseDTO>> getProductsByCategory(
			@Parameter(in = ParameterIn.PATH,
					description = "categoryId to get products") @PathVariable Integer categoryId,
			@Parameter(in = ParameterIn.QUERY,
					description = "the page number to retrieve products if there are more than one page of data",
					schema = @Schema(minimum = "1", defaultValue = "1")) @RequestParam(required = false,
							defaultValue = "1", value = "page") @Min(value = 1) int pageNumber,
			@Parameter(in = ParameterIn.QUERY,
					...
					schema = @Schema(minimum = "5", maximum = "20", defaultValue = "5")) @RequestParam(required = false,
							defaultValue = "5", value = "size") @Min(value = 5) @Max(value = 20) int pageSize,
			@Parameter(in = ParameterIn.QUERY, description = "search keyword to search for products") @RequestParam(
					value = "search", required = false, defaultValue = "") String searchKey,
			@Schema(hidden = true) @RequestHeader(name = "userRole") String role) throws JsonProcessingException {

		Page<ProductResponseDTO> productsInCategory = this.productService.findProductsByCategory(categoryId, pageNumber,
				pageSize, searchKey, role);
		if (productsInCategory.getContent().isEmpty()) {
			return new ResponseEntity<>(HttpStatus.NO_CONTENT);
		}
		CollectionModel<ProductResponseDTO> productCollectionModel = addPageMetadata(productsInCategory.getContent(),
				productsInCategory, categoryId, searchKey, role);
	...
	@Operation(summary = "Update product by Id",
			description = "A PUT request to update product by Id, accessible by <b> ADMINS </b> only",
			tags = { "Product " + "Service" })
	@ApiResponses(value = { @ApiResponse(responseCode = "200", description = "Successfully found the product"),
			@ApiResponse(responseCode = "400", description = "Input validation failed"),
			@ApiResponse(responseCode = "404", description = "Product Not Found"),
	...
	public ResponseEntity<EntityModel<ProductResponseDTO>> updateProduct(
			@Schema(hidden = true) @RequestHeader(name = "loggedInUser") String username,
			@Schema(hidden = true) @RequestHeader(name = "userRole") String role, @PathVariable Integer productId,
			@RequestBody ProductRequestDTO productRequestDTO) throws Exception {

		ProductResponseDTO productResponseDTO = this.productService.updateProductById(productId, productRequestDTO,
				username, role);
		LOGGER.info("Product with Id: {} updated successfully", productId);
		EntityModel<ProductResponseDTO> entityProductModel = this.productAssembler.toModel(productResponseDTO);
	 ...
	 */
	@Operation(summary = "Reduce product count",
			description = "A PUT request to order a specific product, accessible by all users",
			tags = { "Product Service" })
	@ApiResponses(value = { @ApiResponse(responseCode = "200", description = "Successfully reduced the count"),
			@ApiResponse(responseCode = "400", description = "Input validation failed"),
			@ApiResponse(responseCode = "401", description = "Unauthorized user"),
			@ApiResponse(responseCode = "404", description = "Product Not Found"),
			@ApiResponse(responseCode = "500", description = "Some error occurred") })
	@PutMapping("/v1/order-product/{productId}")
	public ResponseEntity<Void> orderProduct(@PathVariable Integer productId, @RequestParam String productSize,
	 ...
	 */
	@Operation(summary = "Export products data in Excel",
			description = "A GET request to download products list "
					+ "in a excel file, accessible only by <b> ADMINS </b>",
			tags = { "Product Service" })
	@ApiResponses(value = { @ApiResponse(responseCode = "200", description = "Successfully downloaded Excel file"),
			@ApiResponse(responseCode = "401", description = "Unauthorized user"),
	...
	public void exportToExcel(HttpServletResponse response,
			@Schema(hidden = true) @RequestHeader(name = "userRole") String role) throws IOException {
	 ...
	 * @throws IOException to handle Input/Output exceptions
	 */
	@Operation(summary = "Export products data in Pdf",
			description = "A GET request to download products list " + "in" + " a Pdf "
	 ...
	 */
	@Operation(summary = "Upload product's images",
			description = "A POST request to upload product images, accessible only by <b> ADMINS </b>",
			tags = { "Product Service" })
	@ApiResponses(value = { @ApiResponse(responseCode = "200", description = "Successfully uploaded images"),
			@ApiResponse(responseCode = "401", description = "Unauthorized user"),
			@ApiResponse(responseCode = "404", description = "Product not found with specified Id"),
			@ApiResponse(responseCode = "500", description = "Some Exception Occurred") })
	@PostMapping(value = "/v1/upload-file/{productId}", consumes = MediaType.MULTIPART_FORM_DATA_VALUE)
	public ResponseEntity<String> uploadFile(@RequestParam(value = "mainImage") MultipartFile mainImage,
			@RequestParam(value = "extraImages") MultipartFile[] extraImages,
			@Parameter(in = ParameterIn.PATH, description = "id of the product") @PathVariable Integer productId,
			@Schema(hidden = true) @RequestHeader(name = "userRole") String role,
			@Schema(hidden = true) @RequestHeader(name = "loggedInUser") String username) throws Exception {
		...
		FileUploadUtil.saveFile(productId,
				StringUtils.cleanPath(Objects.requireNonNull(mainImage.getOriginalFilename())), mainImage, role);
		...
		ProductRequestDTO productRequestDTO = this.modelMapper.map(productResponseDTO, ProductRequestDTO.class);
	 ...
	 */
	@Operation(summary = "Download image of product by file name",
			description = "A GET request to get image, accessible by all users", tags = { "Product Service" })
	@ApiResponses(value = { @ApiResponse(responseCode = "200", description = "Successfully downloaded image"),
	...
	public ResponseEntity<?> downloadFile(
			@Parameter(in = ParameterIn.PATH,
					description = "id of the product") @PathVariable("productId") Integer productId,
			@Parameter(in = ParameterIn.PATH,
					description = "product imageName to download") @PathVariable("imageName") String imageName) {
	...
	@Operation(summary = "Delete product by product Id",
			...
			tags = { "Product Service" })
	@ApiResponses(value = { @ApiResponse(responseCode = "200", description = "Successfully deleted product"),
			@ApiResponse(responseCode = "404", description = "Product not found"),
			@ApiResponse(responseCode = "401", description = "Unauthorized user"),
			@ApiResponse(responseCode = "500", description = "Some Exception Occurred") })
	@DeleteMapping("/v1/delete/{productId}")
	public ResponseEntity<Void> deleteProduct(@PathVariable Integer productId,
			@Schema(hidden = true) @RequestHeader(name = "userRole") String role) throws Exception {
	...
	/**
	 * This function takes in a list of products and a page of products and returns a
	 * CollectionModel of ProductResponseDTOs that includes pagination metadata. It
	 * iterates over the products list and adds the links from the HATEOAS EntityModel to
	 * each ProductResponseDTO.
	 ...
	 */
	private CollectionModel<ProductResponseDTO> addPageMetadata(List<ProductResponseDTO> products,
			Page<ProductResponseDTO> page, Integer categoryId, String searchKey, String role)
		...
		PagedModel.PageMetadata pageMetadata = new PagedModel.PageMetadata(pageSize, pageNumber, totalElements,
				totalPages); // create a new page metadata object
		...
		if (categoryId == 0) {
			collectionModel.add(linkTo(
					methodOn(ProductServiceController.class).getAllProducts(pageNumber, pageSize, searchKey, role))
				.withSelfRel());
			if (pageNumber > 1) {
				// add link to first page if the current page is not the first one
				collectionModel
					.add(linkTo(methodOn(ProductServiceController.class).getAllProducts(1, pageSize, searchKey, role))
						.withRel(IanaLinkRelations.FIRST));
				// add link to the previous page if the current page is not the first one
				collectionModel.add(linkTo(methodOn(ProductServiceController.class).getAllProducts(pageNumber - 1,
						pageSize, searchKey, role))
			...
			if (pageNumber < totalPages) {
				...
				collectionModel.add(linkTo(methodOn(ProductServiceController.class).getAllProducts((int) totalPages,
						pageSize, searchKey, role))
					.withRel(IanaLinkRelations.LAST));
			}
		}
		else {
			collectionModel.add(linkTo(methodOn(ProductServiceController.class).getProductsByCategory(categoryId,
					pageNumber, pageSize, searchKey, role))
				.withSelfRel());
			if (pageNumber > 1) {
				// add link to first page if the current page is not the first one
				collectionModel.add(linkTo(methodOn(ProductServiceController.class).getProductsByCategory(categoryId, 1,
						pageSize, searchKey, role))
					.withRel(IanaLinkRelations.FIRST));
				// add link to the previous page if the current page is not the first one
				collectionModel.add(linkTo(methodOn(ProductServiceController.class).getProductsByCategory(categoryId,
						pageNumber - 1, pageSize, searchKey, role))
					.withRel(IanaLinkRelations.PREV));
			}
			if (pageNumber < totalPages) {
				// add link to next page if the current page is not the last one
				...
				// add link to last page if the current page is not the last one
				collectionModel.add(linkTo(methodOn(ProductServiceController.class).getProductsByCategory(categoryId,
						(int) totalPages, pageSize, searchKey, role))
					.withRel(IanaLinkRelations.LAST));
			}
		}
