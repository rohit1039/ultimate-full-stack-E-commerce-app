...
/**
 * This service class contains the business logic for user authentication and account
 * management. It interacts with the database and sends email notifications.
 */
@Service
...
public class AuthenticationServiceImpl implements AuthenticationService {
	 ...
	 */
	@Override
	public UserDTOResponse createUser(UserDTO userDTO) throws DuplicateUsernameException, InterruptedException {
	 ...
	 * @return the user data with additional fields such as first name, last name, and
	 * avatar name
	...
	public ForgotPasswordResponse forgotPassword(ForgotPasswordDTO forgotPasswordDTO)
		...
		if (!isNull(userInDB)) {
			...
			if (forgotPasswordDTO.getNewPassword().matches(REGEX)) {
				if (this.passwordEncoder.matches(forgotPasswordDTO.getOldPassword(), userInDB.getPassword())) {
				...
				throw new PasswordCriteriaException("Password should be of minimum 8 characters and should contain "
	...
	public UserDTOResponse checkDuplicateEmailIDs(UserDTO userDTO) throws DuplicateUsernameException {
		...
		if (isNull(user)) {
			...
			throw new DuplicateUsernameException("User already exist with emailID: " + userDTO.getEmailId());
		}
	}
	...
	private String getMailBody(String fullName) {
		...
		String body = "<div style=\"font-family: Helvetica,Arial,sans-serif;min-width:1000px;overflow:auto;line-height:1.5;background-color:#F2F2F2;\"> "
				+ "<div style=\"margin:50px auto 50px auto;width:85%;padding:10px 10px;background-color:#FFFFFF;\">"
				+ "<p>" + "Dear " + "<b>" + fullName + "</b>," + "</p> "
				+ "<p> This mail is to inform you that, your registration with us is <b style='color:green'>SUCCESSFUL</b>. </p> "
				+ "<p> Welcome, to the India's No. 1 clothing brand, where you can find products at a very budget friendly price range."
				+ "<p> Make sure to add your products to cart before it gets out of stock due to high demand. </p>"
				+ "<p> Please feel free to browse and order your choice of products. </p>" + " <br />" + "<br/>"
				+ "<div style='margin-top:2px;line-height:0'><p>Best Regards,</p><br/><p>Rohit Parida</p><br/><p>Ph. - (+91) 7978251158</p></div></div></div>";
