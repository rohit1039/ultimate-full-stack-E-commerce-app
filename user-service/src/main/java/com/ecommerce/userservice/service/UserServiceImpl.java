...
public class UserServiceImpl implements UserService {
	...
	public UserDTOResponse updateUser(UpdateUserDTO updateUserDTO, String username) {
		...
		if (userInDB != null) {
			if (userInDB.isEnabled()) {
				...
				User setUpdatedUser = userInDB.toBuilder()
	...
	public List<UserDTOResponse> listAll() {
		...
		return users.stream().map(u -> this.modelMapper.map(u, UserDTOResponse.class)).collect(Collectors.toList());
	}
	...
	public Page<UserDTOResponse> getPageOfFilteredUsers(Pageable pageable, String searchKey) {
		...
		Query query = new Query();
		query.with(pageable)
			.addCriteria(new Criteria().orOperator(Criteria.where("emailID").regex(searchKey, "i"),
					Criteria.where("firstName").regex(searchKey, "i"), Criteria.where("lastName").regex(searchKey, "i"),
					Criteria.where("role").regex(searchKey, "i")));
		List<User> users = this.mongoTemplate.find(query, User.class);
		Page<UserDTOResponse> page = PageableExecutionUtils
			.getPage(users, pageable, () -> this.mongoTemplate.count(Query.of(query).limit(-1).skip(-1), User.class))
