...
@Tag(name = "Authentication Service", description = "All the users should use this service for authentication"
...
public class AuthenticationController {
	 ...
	 */
	@Operation(summary = "Register a new user to be managed by the admins",
	...
	@ApiResponses(value = {
			...
			@ApiResponse(responseCode = "409", description = "Duplicate emailID",
					content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE,
							schema = @Schema(implementation = ExceptionInResponse.class))),
	...
	private ResponseEntity<EntityModel<UserDTOResponse>> registerUser(@Valid @RequestBody UserDTO userDTO)
		...
		this.authenticationService.sendMail(userDTO.getEmailId(), fullName, "User Registration Success 🎉");
	 ...
	 * @return JWTResponse
	 */
	@Operation(summary = "Login to get a token for authentication",
			description = "A POST request to authenticate users accessible by <b>REGISTERED USERS</b>",
			tags = { "Authentication " + "Service" })
	@ApiResponses(value = {
			...
			@ApiResponse(responseCode = "500", description = "Some Exception Occurred",
					content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE,
							schema = @Schema(implementation = ExceptionInResponse.class))) })
	@PostMapping("/login")
	private ResponseEntity<JSONObject> userLogin(@Valid @RequestBody LoginDTO loginDTO) throws BadCredentialsException {
	...
	/**
	 * This API is used to enable the user update their password by satisfying the
	 * password criteria
	 * @param forgotPasswordDTO Request to reset password
	 * @return either JWTResponse or error Message with status codes
	 */
	@Operation(summary = "Reset password for user, and use the updated password for future access",
			description = "A PATCH request to update user's password accessible by <b>REGISTERED USERS</b>",
			tags = { "Authentication Service" })
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully updated user's password'",
					content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE,
	...
	public ResponseEntity<ForgotPasswordResponse> forgotPassword(@RequestBody ForgotPasswordDTO forgotPasswordDTO) {
		...
		ForgotPasswordResponse forgotPasswordResponse = this.authenticationService.forgotPassword(forgotPasswordDTO);
