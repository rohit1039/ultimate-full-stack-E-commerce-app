...
@Tag(name = "User Service",
		description = "Authorized users should use this service to get user's details only, "
...
public class UserServiceController {
	 ...
	 */
	@Operation(summary = "Get user by username managed by admins",
	...
	public ResponseEntity<EntityModel<UserDTOResponse>> getUserByEmail(@Parameter(required = true,
			in = ParameterIn.PATH, description = "emailId of the user") @PathVariable("username") String emailId) {
	 ...
	 */
	@Operation(summary = "Get all users managed by admins",
			description = "A GET request to get all users accessible by <b>ADMINS</b>", tags = { "User Service" })
	@ApiResponses(value = {
			...
			@ApiResponse(responseCode = "401", description = "Unauthorized Access",
					content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE,
							schema = @Schema(implementation = ExceptionInResponse.class))),
			@ApiResponse(responseCode = "204", description = "No users found on this page",
	...
	public ResponseEntity<CollectionModel<UserDTOResponse>> getAllUsers(
			@Parameter(in = ParameterIn.QUERY,
					description = "the page number to retrieve users if there are more than one page of data",
					schema = @Schema(minimum = "1", defaultValue = "1")) @RequestParam(required = false,
							defaultValue = "1", value = "page") @Min(value = 1) int pageNumber,
			@Parameter(in = ParameterIn.QUERY,
					description = "the number of items per page, or the maximum number of users listed in response",
			...
			@Parameter(in = ParameterIn.QUERY,
					...
					schema = @Schema()) @RequestParam(required = false, defaultValue = "",
							value = "search") String query) {

		Page<UserDTOResponse> users = this.userService.usersList(pageNumber, pageSize, query);
		CollectionModel<UserDTOResponse> collectionModel = addPageMetadata(users.getContent(), users, query);
	 ...
	 */
	@Operation(summary = "Update user by username managed by all users",
			description = "A PUT request to update user accessible by all <b>USERS</b>", tags = { "User " + "Service" })
	@ApiResponses(value = {
	...
	public ResponseEntity<EntityModel<UserDTOResponse>> userUpdate(@Valid @RequestBody UpdateUserDTO updateUserDTO) {

		UserDTOResponse updatedUser = this.userService.updateUser(updateUserDTO, updateUserDTO.getEmailID());
	 ...
	 */
	@Operation(summary = "Update user by user's role",
	...
	public ResponseEntity<EntityModel<UserDTOResponse>> updateUserRole(@PathVariable String username,
	...
	@Operation(summary = "Delete user by username",
			description = "A DELETE request to delete user accessible by <b>ADMINS</b>", tags = { "User " + "Service" })
	@ApiResponses(value = {
	 ...
	 */
	@Operation(summary = "Upload user's display picture",
			description = "A POST request to upload image accessible by all users", tags = { "User Service" })
	@ApiResponses(value = { @ApiResponse(responseCode = "200", description = "Successfully uploaded image"),
			@ApiResponse(responseCode = "500", description = "Some Exception Occurred") })
	 ...
	 */
	@Operation(summary = "Download image of user by file code",
			description = "A GET request to get image accessible by <b>ADMINS</b>", tags = { "User Service" })
	...
	@Operation(summary = "Export user's data in Excel",
			description = "A GET request to download user's list in a " + "Excel file accessible by <b>ADMINS</b>",
			tags = { "User Service" })
	@ApiResponses(value = { @ApiResponse(responseCode = "200", description = "Successfully found all the users"),
			@ApiResponse(responseCode = "401", description = "Unauthorized Access",
					content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE,
							schema = @Schema(implementation = ExceptionInResponse.class))),
			@ApiResponse(responseCode = "500", description = "Some Exception Occurred",
	...
	@Operation(summary = "Export user's data in Pdf",
			description = "A GET request to download user's list in a Pdf " + "file accessible by <b>ADMINS</b>",
			tags = { "User Service" })
	@ApiResponses(value = { @ApiResponse(responseCode = "200", description = "Successfully found all the users"),
			@ApiResponse(responseCode = "401", description = "Unauthorized Access",
					content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE,
	 ...
	 */
	@Operation(security = @SecurityRequirement(name = "JWT_token"),
			summary = "Get current loggedIn user's role accessible by all users",
			description = "A GET request to get user's role", tags = { "User Service" })
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Successfully found loggedIn user's role'",
					content = @Content(mediaType = MediaType.TEXT_PLAIN_VALUE,
							schema = @Schema(implementation = String.class))),
			@ApiResponse(responseCode = "401", description = "Unauthorized Access",
					content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE,
	...
	public ResponseEntity<String> getCurrentUserRole(@AuthenticationPrincipal CustomUserDetails userDetails) {
	 ...
	 */
	private CollectionModel<UserDTOResponse> addPageMetadata(List<UserDTOResponse> users, Page<UserDTOResponse> page,
			String searchKey) {
		...
		collectionModel.add(linkTo(methodOn(UserServiceController.class).getAllUsers(pageNumber, pageSize, searchKey))
			.withSelfRel());
		if (pageNumber > 1) {
			// add link to first page if the current page is not the first one
			collectionModel.add(linkTo(methodOn(UserServiceController.class).getAllUsers(1, pageSize, searchKey))
				.withRel(IanaLinkRelations.FIRST));

			// add link to the previous page if the current page is not the first one
			collectionModel
				.add(linkTo(methodOn(UserServiceController.class).getAllUsers(pageNumber - 1, pageSize, searchKey))
		...
		if (pageNumber < totalPages) {
			...
			collectionModel
				.add(linkTo(methodOn(UserServiceController.class).getAllUsers((int) totalPages, pageSize, searchKey))
