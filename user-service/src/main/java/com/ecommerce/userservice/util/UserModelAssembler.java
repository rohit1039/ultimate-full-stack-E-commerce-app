...
public class UserModelAssembler implements RepresentationModelAssembler<UserDTOResponse, EntityModel<UserDTOResponse>> {
	...
	public EntityModel<UserDTOResponse> toModel(UserDTOResponse userApiResponse) {
		 ...
		 */
		String decodedValue = CustomURLDecoder.decodeValue(
				linkTo(methodOn(UserServiceController.class).getUserByEmail(userApiResponse.getEmailId())).withSelfRel()
					.getHref());

		return EntityModel.of(userApiResponse,
				linkTo(methodOn(UserServiceController.class).getUserByEmail(userApiResponse.getEmailId())).withSelfRel()
