...
public class CategoryResponseDTO {
  ...
  /**
   * We have parent category, so for parent categories there will be no parent present in this case
   * the parent will be null. So when parent is null we should exclude it from our JSON response
   */
  @JsonInclude(JsonInclude.Include.NON_NULL)
  private Category parent;

  /**
   * Basically it will not display the hasChildren field when it is false, will display only if it
   * is true
   */
