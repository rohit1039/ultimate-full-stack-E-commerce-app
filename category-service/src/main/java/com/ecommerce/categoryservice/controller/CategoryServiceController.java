...
@Tag(name = "Category Service",
		description = "Clients should use this service to get category's details only, "
...
public class CategoryServiceController {
	...
	@Operation(summary = "Create Category",
			description = "A POST request to create & save categories, accessible only by <b> ADMINS </b>",
			tags = "Category Service")
	@ApiResponses(value = { @ApiResponse(responseCode = "201", description = "Successfully created the category"),
	...
	@PostMapping("/v1/add")
	public ResponseEntity<MappingJacksonValue> createCategory(@Valid @RequestBody CategoryRequestDTO categoryDTO,
		...
		CategoryResponseDTO savedCategory = this.categoryService.saveCategory(categoryDTO, username, role);
	 ...
	 */
	@Operation(summary = "Get all managed categories",
			description = "A GET request to get list of categories, accessible by all users", tags = "Category Service")
	@ApiResponses(value = { @ApiResponse(responseCode = "200", description = "Successfully found the categories"),
	...
	public ResponseEntity<MappingJacksonValue> getAllCategories(
			...
			@Parameter(in = ParameterIn.QUERY, description = "filter with name of the category") @RequestParam(
					value = "category_name", required = false, defaultValue = "") String categoryName,
			@Parameter(in = ParameterIn.QUERY, description = "filter with category created by") @RequestParam(
					value = "created_by", required = false, defaultValue = "") String createdBy,
			@Parameter(in = ParameterIn.QUERY, description = "filter with category enabled or disabled") @RequestParam(
					value = "enabled", required = false, defaultValue = "") String enabled) {
		...
		List<CategoryResponseDTO> savedCategory = this.categoryService.findAllCategories(filterFields, role);
		CollectionModel<EntityModel<CategoryResponseDTO>> collectionModel = categoryModelAssembler
			.toCollectionModel(savedCategory);
		SimpleFilterProvider simpleFilterProvider = new SimpleFilterProvider().setFailOnUnknownId(false);
		FilterProvider filter = simpleFilterProvider.addFilter("filter_parent",
	...
	@Operation(summary = "Get category by Id",
			...
			tags = "Category " + "Service")
	@ApiResponses(value = { @ApiResponse(responseCode = "200", description = "Successfully found the category"),
			@ApiResponse(responseCode = "404", description = "No categories found"),
			@ApiResponse(responseCode = "401", description = "Unauthorized user"),
			@ApiResponse(responseCode = "500", description = "Some error occurred") })
	@GetMapping("/v1/get/{categoryId}")
	public ResponseEntity<MappingJacksonValue> getCategoryById(
			@Parameter(in = ParameterIn.PATH, description = "categoryId to get category") @Min(value = 1,
					message = "cannot be zero or negative") @PathVariable Long categoryId) {
	 ...
	 */
	@Operation(summary = "Export categories data in Excel",
			description = "A GET request to download categories list "
					+ "in a excel file, accessible only by <b> ADMINS </b>",
			tags = { "Category Service" })
	@ApiResponses(value = { @ApiResponse(responseCode = "200", description = "Successfully found all the categories"),
			@ApiResponse(responseCode = "401", description = "Unauthorized user"),
	...
	public void exportToExcel(HttpServletResponse response,
		...
		List<CategoryResponseDTO> listCategories = this.categoryService.findAllCategories(new HashMap<>(), "");
	 ...
	 * @throws IOException to handle Input/Output exceptions
	 */
	@Operation(summary = "Export categories data in Pdf",
			description = "A GET request to download categories list " + "in" + " a Pdf "
	...
	@Operation(summary = "Update Category",
			...
			tags = { "Category " + "Service" })
	@ApiResponses(value = { @ApiResponse(responseCode = "200", description = "Successfully updated the category"),
			@ApiResponse(responseCode = "400", description = "Input validation failed"),
	...
	public ResponseEntity<MappingJacksonValue> updateCategory(
		...
		CategoryResponseDTO categoryResponseDTO = this.categoryService.updateCategory(categoryId, categoryRequestDTO,
				username, role);
		MappingJacksonValue map = modelToJackson(categoryResponseDTO, ignorableFieldNamesInGetCategoryById);
		return new ResponseEntity<>(map, HttpStatus.OK);
	}
	 ...
	 * @return status code
	 */
	@Operation(summary = "Update status of a category",
			description = "A PATCH request to update a category, accessible only by <b> ADMINS </b> " + "categories",
			tags = { "Category Service" })
	@ApiResponses(value = { @ApiResponse(responseCode = "200", description = "Successfully enabled/disabled category"),
			@ApiResponse(responseCode = "401", description = "Unauthorized user"),
			@ApiResponse(responseCode = "500", description = "Some Exception Occurred") })
	...
	/**
	 * This method takes a CategoryResponseDTO object and returns a MappingJacksonValue
	 * object that contains the serialized EntityModel of the CategoryResponseDTO object.
	 * The method uses the SimpleBeanPropertyFilter class to filter out the properties
	 * that should not be serialized.
	 * @param savedCategory the CategoryResponseDTO object to be serialized
	 * @param ignorableFields a set of field names that should be ignored during
	 * serialization
	 * @return a MappingJacksonValue object that contains the serialized EntityModel of
	 * the CategoryResponseDTO object
	 */
	private MappingJacksonValue modelToJackson(CategoryResponseDTO savedCategory, Set<String> ignorableFields) {

		EntityModel<CategoryResponseDTO> entityModel = this.categoryModelAssembler.toModel(savedCategory);
		SimpleFilterProvider simpleFilterProvider = new SimpleFilterProvider().setFailOnUnknownId(false);
		FilterProvider filter = simpleFilterProvider.addFilter("filter_parent",
				SimpleBeanPropertyFilter.serializeAllExcept(ignorableFields));
