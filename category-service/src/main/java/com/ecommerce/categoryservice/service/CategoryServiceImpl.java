...
public class CategoryServiceImpl implements CategoryService {
	 ...
	 */
	@Override
	public CategoryResponseDTO saveCategory(CategoryRequestDTO categoryDTO, String username, String role) {
		...
		if (categoriesIsUnique(0L, categoryDTO.getCategoryName())) {
			...
			if (isNull(categoryDTO.getParentCategoryName()) || categoryDTO.getParentCategoryName().isEmpty()) {
				savedCategory = this.categoryRepository.save(category);
				LOGGER.info("***** Parent category saved successfully *****");
			}
			else {
				Category parentCategory = this.categoryRepository
					.findByParentCategory(categoryDTO.getParentCategoryName().replaceAll("--", ""));
	 ...
	 */
	@Override
	public List<CategoryResponseDTO> findAllCategories(Map<String, Object> filterFields, String role) {
		// Get the categories
	 ...
	 */
	@Override
	public CategoryResponseDTO updateCategory(Long categoryId, CategoryRequestDTO categoryDTO, String username,
			String role) {
		// Check if the user has the required role
		if (!isNull(role) && role.equals("ROLE_USER")) {
		...
		// Get the category
		Category categoryInDB = this.categoryRepository.findById(categoryId)
	...
	public void updateStatusCategory(Long categoryId, boolean status, String username, String role) {
		...
		LOGGER.info("***** Category with ID: " + categoryId + " status updated: {} successfully *****", status);
	}
	 ...
	 */
	public List<CategoryResponseDTO> listHierarchicalCategories(List<Category> rootCategories, String role) {
	...
	public Boolean categoriesIsUnique(Long categoryId, String categoryName) {
		...
		Category categoryInDB = this.categoryRepository.findByCategoryName(categoryName.replaceAll("--", ""));
		if (categoryId == 0) {
